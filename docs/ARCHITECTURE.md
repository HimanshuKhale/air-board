...
Smart Shapes uses deterministic resampling, closure, path length, straightness, perimeter fit and corner coverage. It runs once on a completed pen stroke, never on video frames. Small/ambiguous marks stay ink. A request-bound preview gives eight seconds to convert or keep; optional delayed auto-conversion is off by default. The anatomical left hand can settle only the active request with a stable V or shaka pose, while the anatomical right hand remains available for handle editing. Raw unmirrored MediaPipe handedness is converted once at the interaction boundary. A large handwritten O can be geometrically indistinguishable from a circle, so the presenter remains responsible for accepting it.
