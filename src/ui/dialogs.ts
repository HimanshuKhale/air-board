...
export function dialogs(): string {
  return `<dialog id="settings-dialog"><div class="dialog-heading"><div><p class="eyebrow">MAKE IT FEEL NATURAL</p><h2>Calibration & settings</h2></div><button type="button" data-action="close-settings" aria-label="Close settings">✕</button></div>
    ...
    <h3>Smart shapes</h3>
    <label class="check-field"><input type="checkbox" data-setting="smartShapes"> Suggest clean shapes after drawing</label>
    <label class="check-field"><input type="checkbox" data-setting="autoConvertShapes"> Auto-convert high-confidence shapes after three seconds</label>
  ...
  </dialog><section id="voice-panel" class="voice-panel" hidden aria-label="AirBoard voice controls"><div class="voice-heading"><b>AirBoard Intelligence</b><button type="button" id="voice-close" aria-label="Close voice panel">×</button></div><p>Audio is sent to the local speech service only while listening. With the OpenAI provider, that service sends each audio segment to OpenAI. Camera and board images are never sent.</p><label class="field">Mode<select id="ai-mode"><option value="off">Off</option><option value="commands">Commands</option><option value="automatic" disabled>Automatic (coming later)</option></select></label><label class="check-field"><input id="command-mode" type="checkbox"> Command Mode (no wake phrase needed)</label><div class="voice-actions"><button type="button" id="mic-toggle">Start microphone</button><button type="button" id="undo-ai">Undo Last AI Action</button></div><label class="field">Type a command<input id="typed-command" type="text" maxlength="500" placeholder="AirBoard, create a flowchart with input, process and output"></label><button type="button" id="send-command">Run command</button><p id="ai-status" role="status">Microphone off</p><p id="ai-transcript" aria-live="polite"></p><ol id="ai-log" aria-label="AI action log"></ol></section><div id="shape-suggestion" class="shape-suggestion" hidden><span id="shape-suggestion-label"></span><button type="button" id="shape-convert">Convert</button><button type="button" id="shape-keep">Keep ink</button></div><div id="toast" class="toast" role="status"></div><div id="hand-cursor" class="hand-cursor" hidden><span></span></div>`;
}
