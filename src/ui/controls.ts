...
export function bindControls(bus: BoardChannel, actions: ControlsActions): void {
  ...
  document.addEventListener('click', event => {
    ...
    switch (action) {
      ...
      case 'object-move': case 'object-move-scale': case 'object-move-rotate': case 'object-full': case 'object-cut':
      ...
      case 'object-apply-cut': actions.applyCut(); break;
      case 'capture-fist-near': actions.calibrateFistDepth('near'); break;
      case 'capture-fist-far': actions.calibrateFistDepth('far'); break;
  ...
  document.addEventListener('input', event => {
    ...
    switch (name) {
      ...
      case 'openPalmHoldMs': case 'palmEraserSize': case 'lassoCloseRadius': case 'lassoHoldMs': case 'penGripHoldMs': case 'fistGrabRadius': case 'twoHandHoldMs': case 'twoHandProximity': case 'confirmationHoldMs': case 'reactionDurationMs': case 'spatialTransformHoldMs': case 'spatialScaleGain': case 'spatialSmoothing': case 'spatialScaleDeadZone': case 'spatialRotationDeadZoneDeg': case 'fistDepthNear': case 'fistDepthFar': sendSettings({ [name]: Number(input.value) }); break;
...
export function updateControls(bus: BoardChannel): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(button => {
    ...
    if (['pen', 'highlighter', 'eraser', 'bg-blank', 'bg-camera', 'bg-image', 'pause', 'shape-scale', 'shape-points', 'resize-proportional', 'resize-free', 'object-move', 'object-move-scale', 'object-move-rotate', 'object-full', 'object-cut'].includes(action ?? '')) button.setAttribute('aria-pressed', String(pressed));
  ...
  const selectionCount = document.getElementById('selection-count'); if (selectionCount) selectionCount.textContent = String(bus.state.selection.length);
